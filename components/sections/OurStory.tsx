...
function TextBlock({ title, text }: { title: string; text: string }) {
  return (
    <div className="w-full max-w-[340px] flex-shrink-0">
      <h3 className="text-amber-900 mb-4" style={{ fontFamily: "'Great Vibes', cursive", fontSize: "clamp(2rem, 4vw, 3rem)" }}>
        {title}
      </h3>
      <p className="text-amber-900/60 leading-relaxed text-sm md:text-base font-light" style={{ fontFamily: "'Montserrat', sans-serif" }}>
        {text}
      </p>
    </div>
  );
...
function ImageBlock({ year }: { year: string }) {
  return (
    <div className="w-full max-w-[340px] aspect-[4/3] rounded-3xl overflow-hidden relative group">
      ...
      <div className="absolute inset-0 flex items-center justify-center border border-amber-900/10 rounded-3xl m-3">
         <span className="text-amber-900/20 text-8xl md:text-9xl font-serif">{year.slice(-2)}</span>
      </div>
...
export default function OurStory() {
  return (
    <section data-section className="relative overflow-hidden bg-white">
      ...
      <div className="max-w-6xl mx-auto w-full px-6 py-24 md:py-32">
        ...
        <AnimatedSection direction="fade" className="w-full">
          <div className="text-center mb-20 md:mb-32">
            <p className="text-amber-700/60 text-xs md:text-sm uppercase tracking-[0.6em] mb-4" style={{ fontFamily: "'Montserrat', sans-serif" }}>
              Our Journey Together
            </p>
            <h2 className="text-amber-900 text-6xl md:text-8xl lg:text-9xl mb-8" style={{ fontFamily: "'Great Vibes', cursive" }}>
              Our Story
            </h2>
            <div className="flex items-center gap-4 justify-center">
              <div className="h-[1px] w-20 bg-gradient-to-r from-transparent to-amber-900/30" />
              <Sparkles size={18} className="text-amber-400/80" />
              <div className="h-[1px] w-20 bg-gradient-to-l from-transparent to-amber-900/30" />
            </div>
          </div>
        </AnimatedSection>
              ...
              <AnimatedSection key={item.year} direction={isImageLeft ? "left" : "right"} delay={0.1 * i} className="w-full">
                ...
                <div className="md:hidden flex flex-col items-center gap-10">
                  <div className="relative">
                    <div className="absolute inset-0 bg-amber-900/5 blur-xl rounded-full" />
                    <span className="relative z-10 text-amber-900/80 text-4xl font-serif tracking-widest">{item.year}</span>
                  </div>
                ...
                <div className="hidden md:grid grid-cols-[1fr_120px_1fr] items-center w-full">
                  ...
                  <div className="relative flex flex-col items-center justify-center min-h-[300px]">
                    <div className="absolute top-0 bottom-0 w-[1px] bg-gradient-to-b from-transparent via-amber-900/20 to-transparent" />
                    <div className="relative z-10 w-16 h-16 rounded-full bg-white flex items-center justify-center border border-amber-900/10 shadow-sm">
                       <span className="text-amber-900/80 text-xs font-bold tracking-tighter uppercase">{item.year}</span>
                    </div>
                  </div>
